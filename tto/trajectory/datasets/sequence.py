...
class ContinuousDataset(SequenceDataset):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # compute normalization params
        self.f_mins = to_torch(self.joined_raw.min(axis=0), device="cpu", dtype=torch.float)
        self.f_maxs = to_torch(self.joined_raw.max(axis=0), device="cpu", dtype=torch.float)

        print("min:", self.f_mins)
        print("max:", self.f_maxs)

    def normalize(self, x, sublice=(None, None)):
        ## enforce batch mode
        if x.ndim == 1:
            x = x[None]
        ...
        f_mins = self.f_mins[start:end].to(x.device)
        f_maxs = self.f_maxs[start:end].to(x.device)

        return ((x - f_mins) / (f_maxs - f_mins)).clamp(min=0., max=1.)

    def denormalize(self, x, sublice=(None, None)):
        ## enforce batch mode
        if x.ndim == 1:
            x = x[None]
    ...
    def __getitem__(self, idx):
        ...
        joined = to_torch(joined, device='cpu', dtype=torch.float)
        ## [ (sequence_length / skip) x observation_dim]
        # joined_discrete = to_torch(joined_discrete, device='cpu', dtype=torch.long).contiguous()
        ...
        ## flatten everything
        joined = self.normalize(joined).view(-1)
        mask = mask.view(-1)
