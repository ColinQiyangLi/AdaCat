...
def forward_new(model, x, max_block=None, allow_crop=True, crop_increment=None, **kwargs):
    '''
        A wrapper around a single forward pass of the transformer.
        Crops the input if the sequence is too long.
...
def sample_new(model, x, temperature=1.0, topk=None, cdf=None, **forward_kwargs):
    # assert topk is None

    dis = forward_new(model, x, **forward_kwargs)
    # print("Sampling with ({}, {})".format("none" if cdf is None else cdf, "none" if topk is None else topk))
    y = dis.sample(zo_ratio=cdf, top_k=topk)
    # y = dis.sample(level=cdf, top_k=topk)
    return y[:, -1:], dis.mean[:, -1]
...
@torch.no_grad()
def sample_n_new(model, x, N, store_mean=False, **sample_kwargs):
    batch_size = len(x)
    means = torch.zeros(batch_size, N, device=x.device)

    for n in range(N):
        t, mean = sample_new(model, x, **sample_kwargs)

        ## append to the sequence and continue
        ## [ batch_size x (sequence_length + n) ]
        if store_mean:
            x = torch.cat((x, mean.unsqueeze(-1)), dim=1)
        else:
