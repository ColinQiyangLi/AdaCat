...
class TruncatedUniform(Uniform):
    def __init__(self, loc, scale, a, b, validate_args=None):
        loc, scale, a, b = broadcast_all(loc, scale, a, b)
        x = (loc - scale).clamp(min=a)
        y = (loc + scale).clamp(max=b)
        self.a, self.b = a, b
        super().__init__(x, y)
    ...
    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        result = (value.clamp(min=self.low, max=self.high) - self.low) / (self.high - self.low)
        return result.clamp(min=0, max=1)

if __name__ == "__main__":
    # test truncated uniform
    d = TruncatedUniform(0.95, 0.1, 0.0, 1.0)
