...
from wavenet_vocoder.util import is_mulaw_quantize, is_mulaw, is_raw, is_quantize, quantize
...
def _process_utterance(out_dir, index, wav_path, text):
    ...
    if is_mulaw_quantize(hparams.input_type):
        ...
        if silence_threshold > 0:
            # [0, quantize_channels)
            out = quantize(wav, hparams.quantize_channels - 1)
            start, end = audio.start_and_end_indices(out, silence_threshold)
            wav = wav[start:end]
        constant_values = quantize(0, hparams.quantize_channels - 1)
        out_dtype = np.int16
