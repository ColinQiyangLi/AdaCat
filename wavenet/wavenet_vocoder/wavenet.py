...
from wavenet_vocoder.util import to_categorical_torch, quantize
...
def scalar_embed_channel_count(embeddings):
    count = 0
    for embedding in embeddings:
        if embedding == "scalar" or embedding == "mulaw":
            count += 1
        elif embedding.startswith("mulaw_quantize_"):
            mu = int(embedding[len("mulaw_quantize_"):])
            count += mu
        elif embedding.startswith("quantize_"):
            count += int(embedding[len("quantize_"):])
        else:
            raise ValueError
    return count


def scalar_embed(x, embeddings):
    """x is (B x 1 x T)"""
    assert x.shape[1] == 1
    assert x.ndim == 3
    emb = []
    for embedding in embeddings:
        if embedding == "scalar":
            emb.append(x)
        elif embedding == "mulaw":
            emb.append(P.mulaw(x))
        elif embedding.startswith("mulaw_quantize_"):
            mu = int(embedding[len("mulaw_quantize_"):])
            x_quant = P.mulaw_quantize(x, mu)
            x_categorical = x_quant.movedim(1, -1)  # (B x T x 1)
            x_categorical = to_categorical_torch(x_categorical, mu)  # (B x T x mu)
            x_categorical = x_categorical.movedim(-1, 1)  # (B x mu x T)
            emb.append(x_categorical)
        elif embedding.startswith("quantize_"):
            k = int(embedding[len("quantize_"):])
            x_quant = quantize(x, k)
            x_categorical = x_quant.movedim(1, -1)  # (B x T x 1)
            x_categorical = to_categorical_torch(x_categorical, k)  # (B x T x mu)
            x_categorical = x_categorical.movedim(-1, 1)  # (B x mu x T)
            emb.append(x_categorical)
        else:
            raise ValueError
...
class WaveNet(nn.Module):
    ...
    def __init__(self, out_channels=256, layers=20, stacks=2,
                 ...
                 scalar_input=False,
                 scalar_embeddings=("scalar",),  # some combination of scalar, mulaw_quantize_<integer mu>
        ...
        self.scalar_embeddings = scalar_embeddings
        ...
        if scalar_input:
            in_channels = scalar_embed_channel_count(scalar_embeddings)
        else:
    ...
    def forward(self, x, c=None, g=None, softmax=False):
        ...
        # Positionally embed input
        if self.scalar_input:
            x = scalar_embed(x, self.scalar_embeddings)  # (B x 1 x in_channels)?
    ...
    def incremental_forward(self, initial_input=None, c=None, g=None,
        ...
        if initial_input is None:
            if self.scalar_input:
                ...
                mid = (self.out_channels - 1) // 2
                initial_input[:, :, mid] = 1  # TODO: is this ok?
        ...
        for t in tqdm(range(T)):
            if test_inputs is not None and t < test_inputs.size(1):
                current_input = test_inputs[:, t, :].unsqueeze(1)  # (B x 1 x C)
            else:
                if t > 0:
                    current_input = outputs[-1].unsqueeze(1)  # (B x 1 x C)

            # Positionally embed input
            if self.scalar_input:
                current_input = scalar_embed(current_input, self.scalar_embeddings)  # (B x 1 x in_channels)?
                assert current_input.ndim == 3
            ...
            if self.scalar_input:
                if self.output_distribution == "Logistic":
                    ...
                    x = sample_from_mix_gaussian(
                        x.view(B, -1, 1), log_scale_min=log_scale_min)
                elif self.output_distribution == "AdaCat":
                    dist = Adacat(x)
                    x = dist.sample()
